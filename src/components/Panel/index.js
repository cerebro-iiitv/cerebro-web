...
class Panel extends React.Component {
	...
	render() {
		...
		let isRegistered =
			this.props.user && _.some(Object.keys(this.props.event.participants), uid => uid === this.props.user.uid);
			...
			<div className={''}>
				<div className="uk-card uk-card-body uk-border-rounded event-panel">
					...
					<p className={'panel-buttons'}>
						{this.props.user &&
							(isRegistered ? (
								<span className={'panel-unregister uk-border-pill'} onclick="return false;">
									Unregister
								</span>
							) : (
								<span className={'panel-register uk-border-pill'} onclick="return false;">
									Register
								</span>
							))}
						<span className={'panel-more uk-border-pill'} onClick={this.navigateToEvent.bind(this)}>
							View Details
						</span>
