...
class Dashboard extends React.Component {
	componentDidMount() {
		...
		this.mobileInput.addEventListener('keyup', event => {
			...
			if (event.keyCode === 13) {
				this.mobileInput.value.toString().length === 10
					...
					<tbody>
						{userRegisteredEvents.map((event, index) => (
							<tr key={event.id}>
								...
								<td>
									<button
										className="uk-button uk-button-secondary primary-color primary-color-border uk-button-small"
										onClick={() => this.handleUnregister(event)}
									>
										Unregister
									</button>
								</td>
